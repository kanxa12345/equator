        ...
        <>
            ...
            <section className='py-10'>
                <div className="container flex flex-col items-center">
                    <h2 className="lg:text-4xl sm:text-3xl text-2xl font-bold mb-8 relative after:absolute after:w-[100%] after:h-[5px] after:bg-red-600 after:content-[''] after:bottom-[-8px] after:rounded-sm after:left-0 before:absolute before:bg-white before:content-[''] before:h-[5px] before:w-[5px] before:bottom-[-8px] before:z-10 animate" data-aos="fade-up" data-aos-duration="1000" data-aos-once='true'>Our Team</h2>
                    <div className="grid xl:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-8 w-full">
                        {TeamData.map((dataItem, index)=>(
                            <div key={index} className='flex flex-col items-start'>
                                <img className='w-full md:h-[300px] h-[250px] object-cover' src={dataItem.imageUrl} alt={dataItem.name} />
                                <h3 className='sm:text-xl text-lg font-semibold'>{dataItem.name}</h3>
                                <p className='sm:text-lg text-base font-medium text-gray-600'>{dataItem.position}</p>
                            </div>
                        ))}
                    </div>
                </div>
